...
class AMPEnv:
    ...
    def step(self, parameters):
        ...
        # Allocate a sample index (for both pass & fail)
        with self.lock:
        ...
        # If simulation failed: create sentinel performance + copy what we can
        if not self.simulation_flag:
            # Sentinel values (you can tweak these)
            sentinel = 1e9  # will become `null` in JSON
            ...
            performance = {
                ...
                'dcgain': sentinel,
                'GBW': sentinel,
                'phase_margin (deg)': sentinel,
                'PSRP': sentinel,
                'PSRN': sentinel,
                'sr': sentinel,
                'setting_time': sentinel,
                'sim_success': 0    # <--- key flag
            }

            # Copy whatever files exist (including *_vars.spice for inputs)
            ...
            with open(os.path.join(sample_folder, "performance.json"), 'w') as f:
                json.dump(performance, f, indent=4)

            logging.info(f"Failed sample {cur_idx}: Reward = -1e10")
            return -1e10

        # ------------------------------
        # If we reach here: simulation succeeded
        # ------------------------------
        info, reward = self.get_performace()

        # Attach success flag into the metrics
        info['sim_success'] = 1
